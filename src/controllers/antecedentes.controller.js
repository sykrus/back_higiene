...
exports.getAllAntecedentesControlados= (req, res) => {
    db.query(`SELECT  antecedentes_controlados.id, antecedentes_controlados.nro_registro, asociado,revision, fecha_vigencia_ant,fecha_revision_ant
  ...
  exports.updateAntecedentesControlados= (req, res) => {
    const { id } = req.params;
    const { nro_registro , asociado , revision,fecha_vigencia_ant,fecha_revision_ant } = req.body;
    const sql = 'UPDATE public.antecedentes_controlados   SET nro_registro  =$1, asociado =$2, revision=$3, fecha_vigencia_ant =$4, fecha_revision_ant =$5  WHERE id=$6';
    db.query(sql, [nro_registro , asociado, revision ,fecha_vigencia_ant,fecha_revision_ant, id], (err, result) => {
      if (err) {
        console.error(err);
        return res.status(500).json({ message: 'Error al actualizar el registro de antecendetes .' });
      }
